...
export async function seed() {
  ...
  try {
    ...
    const nikeResults = await db.insert(brands).values([
      {
        name: 'Nike',
        slug: 'nike',
    ...
    const nike = (nikeResults as Array<{ id: string; name: string; slug: string; logoUrl?: string }>)[0];
    ...
    const menGender = (genderResults as Array<{ id: string; label: string; slug: string }>)[0];
    const womenGender = (genderResults as Array<{ id: string; label: string; slug: string }>)[1];
    const unisexGender = (genderResults as Array<{ id: string; label: string; slug: string }>)[2];
    ...
    const shoesCategory = (categoryResults as Array<{ id: string; name: string; slug: string; parentId?: string }>)[0];
    const clothingCategory = (categoryResults as Array<{ id: string; name: string; slug: string; parentId?: string }>)[1];
    const accessoriesCategory = (categoryResults as Array<{ id: string; name: string; slug: string; parentId?: string }>)[2];

    const subcategoryResults = await db.insert(categories).values([
      { name: 'Running Shoes', slug: 'running-shoes', parentId: shoesCategory.id },
      { name: 'Basketball Shoes', slug: 'basketball-shoes', parentId: shoesCategory.id },
      { name: 'Casual Shoes', slug: 'casual-shoes', parentId: shoesCategory.id },
    ]).returning();
    const runningShoesCategory = (subcategoryResults as Array<{ id: string; name: string; slug: string; parentId?: string }>)[0];
    const basketballShoesCategory = (subcategoryResults as Array<{ id: string; name: string; slug: string; parentId?: string }>)[1];
    const casualShoesCategory = (subcategoryResults as Array<{ id: string; name: string; slug: string; parentId?: string }>)[2];

    console.log('🎨 Seeding colors...');
    const colorData = await db.insert(colors).values([
    ...
    const sizeData = await db.insert(sizes).values([
    ...
    const airMaxCollection = (collectionResults as Array<{ id: string; name: string; slug: string }>)[0];
    const jordanCollection = (collectionResults as Array<{ id: string; name: string; slug: string }>)[1];
    const reactCollection = (collectionResults as Array<{ id: string; name: string; slug: string }>)[2];
    ...
    const productData = [
      ...
      },
      {
        ...
        description: 'The Nike React Element 55 combines lightweight Nike React foam.',
        categoryId: runningShoesCategory.id,
        genderId: unisexGender.id,
        brandId: nike.id,
        isPublished: true,
      ...
      {
        name: 'Air Max Plus',
        description: 'The Nike Air Max Plus delivers a bold look with wavy design lines.',
        categoryId: runningShoesCategory.id,
        genderId: unisexGender.id,
        brandId: nike.id,
        isPublished: true,
      ...
      {
        name: 'Air Max 1',
        description: 'The Nike Air Max 1 brings the original visible Air cushioning.',
        categoryId: runningShoesCategory.id,
        genderId: unisexGender.id,
        brandId: nike.id,
        isPublished: true,
      ...
      return db.insert(productImages).values([
        {
          ...
          url: `https://static.nike.com/a/images/t_PDP_1728_v1/f_auto,q_auto:eco/product-${index + 1}-1.jpg`,
          isPrimary: true,
        },
        {
          productId: product.id,
          url: `https://static.nike.com/a/images/t_PDP_1728_v1/f_auto,q_auto:eco/product-${index + 1}-2.jpg`,
          isPrimary: false,
        },
      ]);
    });
    ...
    const variantPromises = insertedProducts.map((product, productIndex) => {
      ...
      productColors.forEach((color, colorIndex) => {
        productSizes.forEach((size) => {
          ...
          variants.push({
            productId: product.id,
            sku: `NIKE-${product.name.replace(/\s+/g, '').toUpperCase()}-${color.slug.toUpperCase()}-${size.slug.toUpperCase()}`,
            price: basePrice.toString(),
            salePrice: colorIndex === 0 ? (basePrice * 0.9).toString() : undefined,
            colorId: color.id,
            sizeId: size.id,
